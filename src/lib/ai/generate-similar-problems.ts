...
// 참고문제(문제 보관함) 기반: 핵심 개념은 유지하고 숫자는 반드시 새로 정하되,
// 소재/시나리오 변형은 원본이 이미 문장제인 경우에만 적용한다 —
// 원본이 단순 비교식/계산식 드릴이면 그 형식 그대로 숫자만 바꾼다 (원장님 요청사항).
const REFERENCE_INSTRUCTIONS = `아래는 선생님이 보관함에 저장해둔 참고문제입니다. 각 문제마다 요청된 개수만큼, 핵심 개념(연산·단원)과 형식은 그대로 유지하되 숫자(값)는 반드시 새로 정한 문제를 만들어주세요.

1. 새 문제의 숫자/값은 반드시 원본과 다른 새 값으로 정하세요 — 원본의 숫자를 그대로 재사용하는 것은 허용되지 않습니다.
2. 새 문제의 형식은 원본 문제 하나하나를 기준으로 판단하세요.
   - 원본이 이미 구체적인 소재·인물·상황이 있는 문장제라면, 새 문제도 소재/배경/캐릭터/맥락을 바꿔서 같은 개념을 다른 방식으로 묻는 문장제로 작성하세요 — 이 경우 숫자만 바꾸고 소재는 그대로 두는 것은 허용되지 않습니다.
   - 원본이 단순 비교식·계산식·빈칸 채우기처럼 소재·인물·상황이 없는 드릴 형식이라면, 새 문제도 억지로 이야기나 등장인물을 만들어내지 말고 원본과 동일한 단순 형식(원본에 있던 지시문이 있다면 그대로 유지)으로, 숫자만 새로 정해 작성하세요.
3. (위 2번에서 문장제로 작성하는 경우에 한해) 같은 source_id에 대해 여러 개(요청된 개수만큼)를 생성할 때는 그 안에서도 각 문제가 서로 뚜렷하게 달라야 합니다. "Two ___ are/have painted/hiked/run/planted..." 처럼 동일한 문장 시작 패턴과 문장 구조를 명사만 바꿔서 반복하지 마세요. 매 문제마다 서로 다른 영역(예: 스포츠, 요리·베이킹, 쇼핑, 여행, 공예, 과학 실험, 보드게임, 학교 행사, 반려동물, 음악, 건축·만들기 등)에서 소재를 고르고, 등장인물 수·이름·서술 방식(1인칭/3인칭, 비교 대상이 사람인지 사물인지 등)과 문장을 시작하는 방식도 문제마다 바꿔서, 같은 세트 안에서 한눈에 봐도 서로 다른 이야기처럼 보이게 만드세요. 다만 요청한 핵심 개념과 난이도는 모든 변형에서 동일하게 유지해야 합니다. 단순 드릴 형식으로 작성하는 문제에는 이 다양성 규칙이 적용되지 않습니다 — 원본과 같은 단순 형식을 그대로 유지하세요.
4. 원래 문제를 그대로 복사하지 마세요 — 숫자와(해당하는 경우) 소재가 모두 바뀐 새 문제여야 합니다.
5. answer_text에는 최종 정답만 간결하게 적으세요(풀이 과정 없이).
6. 원본 정답이 제공되어 있으면 그것이 맞는지 검산한 뒤 새 문제의 정답을 계산하세요. 원본 정답이 제공되지 않았으면 직접 계산해서 검산한 뒤 답을 작성하세요. 정답이 틀리면 이 기능의 목적 자체가 무너집니다.
7. source_id에는 입력에 주어진 source_id 값을, source_kind에는 "reference"를 그대로 돌려주세요.
8. 각 항목에 대해 요청된 개수만큼 정확히 생성하세요.
9. 아래 "원래 문제"가 한국어로 되어 있더라도, 새로 생성하는 problem_text와 answer_text는 반드시 영어로만 작성하세요.
10. "원래 보기"가 주어진 문제는 객관식입니다 — 새로 만드는 문제도 반드시 객관식으로 만드세요. 보기 개수와 라벨 체계는 원본과 동일하게 유지하되 값(오답 보기 포함)은 새 문제에 맞게 새로 지어내세요(낡은 보기를 그대로 남기지 마세요). options에 담고, problem_text에는 보기를 절대 포함하지 마세요. correct_option에는 새로 만든 정답 보기의 label을, answer_text에는 그 보기의 text와 동일한 값을 넣어 서로 일치시키세요.
11. "원래 보기"가 없으면(서술형) 새 문제도 서술형이어야 합니다 — options와 correct_option은 반드시 null로 두세요.`;
