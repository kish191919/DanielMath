...
// Mirrors src/app/api/grading/run/route.ts. Must be a literal here (not an
// imported constant) — Next.js reads route segment config via static
// analysis, not module evaluation, so an imported identifier fails the
// build with "Invalid segment configuration export". Keep this in sync
// with GRADING_ROUTE_MAX_DURATION_S in grading-config.ts.
