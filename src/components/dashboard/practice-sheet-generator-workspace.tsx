...
// extends it with a second, concept-filterable section for the global
// reference bank, so a teacher can mix a student's wrong answers with
// reusable reference problems in one generation call.
export function PracticeSheetGeneratorWorkspace({
  ...
  const filteredReferences = React.useMemo(
    () =>
      conceptFilter
        ? referenceProblems.filter((p) => p.concept_id === conceptFilter)
        : referenceProblems,
    [referenceProblems, conceptFilter],
  );
      ...
      <section>
        ...
        {referenceProblems.length === 0 ? (
          ...
          <div className="mt-3 space-y-2">
            {filteredReferences.map((problem) => (
              <label
                key={problem.id}
                className="flex items-start gap-3 rounded-2xl border border-navy-100 bg-white p-4 shadow-sm"
              >
                ...
                <div className="flex-1">
                  <p className="text-xs font-semibold uppercase tracking-wide text-navy-500">
                    {conceptById.get(problem.concept_id ?? "")?.label_ko ?? "미분류"}
                  </p>
