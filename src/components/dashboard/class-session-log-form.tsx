...
import { ATTENDANCE_STATUSES, ATTENDANCE_LABELS } from "@/lib/class-sessions/schema";
...
export function ClassSessionLogForm({
  ...
  return (
    ...
    >
      ...
      {roster.length > 0 && (
        <div>
          <p className="mb-2 text-sm font-medium text-navy-800">출결 / Attendance</p>
          <div className="space-y-2">
            {roster.map(({ enrollmentId, student }) => (
              <div
                key={enrollmentId}
                className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-navy-100 px-3 py-2"
              >
                <span className="text-sm text-navy-900">{student.full_name}</span>
